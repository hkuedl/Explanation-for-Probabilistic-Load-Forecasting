...
def main(args):
    ...
    for noise_type in tqdm(['oridinary','random','student']):
        ...
        if noise_type =='oridinary':
            ...
            test_X_ex_fu15 = torch.Tensor(np.load('./process_data/'+name+'/test_X_ex_fu_15%.npy')).to(device)
            test_X_ex_fu20 = torch.Tensor(np.load('./process_data/'+name+'/test_X_ex_fu_20%.npy')).to(device)
            test_X_ex_fu25 = torch.Tensor(np.load('./process_data/'+name+'/test_X_ex_fu_25%.npy')).to(device)

        elif noise_type == 'random':
            test_X_ex_fu10 = torch.Tensor(np.load('./process_data/'+name+'/test_X_ex_fu_10%_f.npy')).to(device)
            test_X_ex_fu15 = torch.Tensor(np.load('./process_data/'+name+'/test_X_ex_fu_15%_f.npy')).to(device)
            test_X_ex_fu20 = torch.Tensor(np.load('./process_data/'+name+'/test_X_ex_fu_20%_f.npy')).to(device)
            test_X_ex_fu25 = torch.Tensor(np.load('./process_data/'+name+'/test_X_ex_fu_25%_f.npy')).to(device)
        elif noise_type == 'student':
            test_X_ex_fu10 = torch.Tensor(np.load('./process_data/'+name+'/test_X_ex_fu_10%_t.npy')).to(device)
            test_X_ex_fu15 = torch.Tensor(np.load('./process_data/'+name+'/test_X_ex_fu_15%_t.npy')).to(device)
            test_X_ex_fu20 = torch.Tensor(np.load('./process_data/'+name+'/test_X_ex_fu_20%_t.npy')).to(device)
            test_X_ex_fu25 = torch.Tensor(np.load('./process_data/'+name+'/test_X_ex_fu_25%_t.npy')).to(device)
        ...
        input_channels=1,
        extra_channels=70 if name =='Spain' else 12,
        hidden_channels=64,
        static_channels=1,
        output_channels=1
        ).to(device)
        ...
        for noise_input in [test_X_ex_fu10,test_X_ex_fu15,test_X_ex_fu20,test_X_ex_fu25]:
            my_list = []
            for i in iter_list:
        ...
        result = result.applymap(lambda x: np.floor(x * 10000) / 10000)
        result.columns = ['0.1', '0.15', '0.2', '0.25']
        result.index = ['Random', 'IG', 'GS', 'DL', 'Ours']
        print(result)
        result.to_json('./result/'+name+'/'+noise_type+'.json', orient='records', lines=True)
        
        # result = {'ours':my_list_list,'IG':IG_list_list,'GS':GS_list_list,'DL':DL_list_list,'Random':random_list_list}
        # with open('./result/'+name+'/'+noise_type+'.pkl', 'wb') as f:
